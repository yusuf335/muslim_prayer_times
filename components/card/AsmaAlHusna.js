...
/*
This component display names of Allah. The function takes one parameter number. If someone pass any number between 0 to 98 then the function returns the name of Allah in that perticular index from the namesOfAllah array. If no number is passed then by default a random number will be generated.

*/

const AsmaAlHusna = ({number = -1}) => {
  ...
  useEffect(() => {
    if (number === -1) {
      setSelectedName(namesOfAllah[Math.floor(Math.random() * 99)]);
    }
  }, []);
  ...
  return (
    <View style={styles.nameContainer}>
      <Text style={styles.arabicName}>{selectedName.name}</Text>
      <Text style={styles.translatedName}>{selectedName.transliteration}</Text>
      <Text style={styles.meaning}>{selectedName.en.meaning}</Text>
    </View>
  );
};
